...
def _leaf_line_end(leaf: Leaf) -> int:
    ...
    # Using ternary operator for more compact code, reducing if-else overhead.
...
def _get_line_range(node_or_nodes: Union[LN, list[LN]]) -> set[int]:
    ...
    # Check for empty lists or singular input at the start to minimize branching.
    if not node_or_nodes:
        return set()

    # Unify node handling by using list-like access for potential singularity.
