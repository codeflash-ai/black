...
def get_leaves_inside_matching_brackets(leaves: Sequence[Leaf]) -> set[LeafID]:
    ...
    if n == 0:
        return set()
    # Tight loop to find first opening bracket (slightly faster than generator)
    start_index = None
    for i in range(n):
        ltype = leaves[i].type
        if ltype in OPENING_BRACKETS:
            start_index = i
            break
    ...
    bracket_stack = []
    ids = set()
    BRACKET_LOOKUP = BRACKET  # Local for performance
    OPENING_LOOKUP = OPENING_BRACKETS
    CLOSING_LOOKUP = CLOSING_BRACKETS
    for i in range(start_index, n):
        leaf = leaves[i]
        ltype = leaf.type
        if ltype in OPENING_LOOKUP:
            bracket_stack.append((BRACKET_LOOKUP[ltype], i))
        elif ltype in CLOSING_LOOKUP:
            if bracket_stack and ltype == bracket_stack[-1][0]:
                _, start = bracket_stack.pop()
                # Add all ids in range [start, i+1]
                ids.update(id(leaves[j]) for j in range(start, i + 1))
            else:
                break
    return ids
