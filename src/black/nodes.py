...
def is_with_or_async_with_stmt(leaf: Leaf) -> bool:
    ...
    if leaf.type == token.NAME and leaf.value == "with":
        parent = leaf.parent
        return parent is not None and parent.type == syms.with_stmt

    if leaf.type == token.ASYNC:
        next_sibling = leaf.next_sibling
        return next_sibling is not None and next_sibling.type == syms.with_stmt
