...
def is_simple_decorator_trailer(node: LN, last: bool = False) -> bool:
    ...
    if len(children) == 2:
        if children[0].type == token.DOT and children[1].type == token.NAME:
            return True
        if last and children[0].type == token.LPAR and children[1].type == token.RPAR:
            return True
    elif last and len(children) == 3:
        if children[0].type == token.LPAR and children[2].type == token.RPAR:
            return True
