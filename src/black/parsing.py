...
def get_grammars(target_versions: set[TargetVersion]) -> list[Grammar]:
    ...
    supports_async_identifiers = supports_feature(target_versions, Feature.ASYNC_IDENTIFIERS)
    supports_pattern_matching = supports_feature(target_versions, Feature.PATTERN_MATCHING)
    supports_async_keywords = supports_feature(target_versions, Feature.ASYNC_KEYWORDS)

    if not supports_async_identifiers and not supports_pattern_matching:
        grammars.append(pygram.python_grammar_async_keywords)
    if not supports_async_keywords:
